...
docker exec $CLIENT runuser -u developer -- python3 /usr/local/bin/test-kerberos.py $( cat ipa-data/admin-password )

docker cp src/www-proxy-saml.conf $WWW:/data/www.conf
docker exec -ti $WWW systemctl restart httpd

# docker cp src/test-saml.py $CLIENT:/usr/local/bin/
docker exec $CLIENT runuser -u developer -- python3 /usr/local/bin/test-saml.py $( cat ipa-data/admin-password )
